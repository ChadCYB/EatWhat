...
$.getJSON( "http://127.0.0.1/~104021132/final/www/api/getStore.php?id="+id , function( data ) {
	store = data;
	console.log(store);
	var phone = (store[0].phone==null)?"尚無":store[0].phone;
	var fork = '<i class="fa fa-cutlery" aria-hidden="true"></i>';
	$("#mytitle").html( fork +' '+ store[0].name +' '+ fork);
	$("#phone").html('電話:<a href="tel:'+phone+'"> '+phone+'</a>');
